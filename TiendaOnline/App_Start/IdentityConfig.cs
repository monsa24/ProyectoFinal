...
namespace TiendaOnline
{
    ...
    public static class IdentitySeeder
    {
        public static void Seed(ApplicationDbContext db)
        {
            ...
            // Admin
            EnsureUser(userManager, "admin@sga.local", "Admin123", "Administrador", "Admin");
            EnsureUser(userManager, "usuario@sga.local", "Admin123", "Asociado", "Usuario");
  

            db.SaveChanges();
        }

        private static void EnsureUser(UserManager<ApplicationUser> um, string email, string pass, string fullName, string role)
        {
            var u = um.FindByEmail(email);
            if (u == null)
            {
                u = new ApplicationUser { UserName = email, Email = email, FullName = fullName };
                um.Create(u, pass);
            }
